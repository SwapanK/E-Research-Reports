...
# =============================================================================
# ---- PER-PLOT X-AXIS CLASS REORDER / RENAME / HIDE ------------------------
# =============================================================================
# Each gallery plot card (region/state/pct x class, e.g. "region"+"CC") gets
# its own reorder/rename/hide editor (opened via a button in its collapsible
...
# is independent - editing the order/labels/visibility for region-CC does
# not affect state-CC or pct-CC.
#
...
#     editable table (Order arrows + locked Name + editable Rename + Show/
#     Hide toggle) and its chip preview row from it - reordering/renaming/
#     hiding inside the modal is pure client-side JS against this shape,
#     only round-tripping to Shiny once, on Apply
...
#     to build scale_x_discrete(limits = name, labels = rename), filtering
#     out any row where visible == FALSE before `limits` is built
# =============================================================================


#' Build the default (identity) class order/rename/visible state for one
#' plot card.
#'
...
#'   - name    : character, the original Description value as it appears
#'               in the data (e.g. "Auto") - never edited by the user,
#'               used to match rows back to the underlying data / scale
#'               limits
#'   - rename  : character, the user-editable display label - defaults to
#'               `name` until the user types something else
#'   - visible : logical, the per-class Show/Hide toggle - defaults to
#'               TRUE (all classes shown) until the user hides one. A
#'               class with visible == FALSE is dropped from the plot's
#'               x-axis entirely by vul_apply_overrides()
#'               (VulsenAPP_plot_functions.R), without touching the
#'               underlying data.
