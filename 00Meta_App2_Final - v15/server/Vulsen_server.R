...
Vulsen_server <- function(id, cart, username) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    output$subperil_ui <- shiny::renderUI({
      ...
      # Keep the current selection if it's still valid for this peril
      # (e.g. re-mount on tab revisit with peril unchanged); otherwise
      # fall back to "AllPeril" or the first available choice.
      current_val <- input$subperil
