...
resultExtractServer <- function(input, output, session) {
  ...
  output$main_preview <- renderDT({
    datatable(main_data(), filter = "top", options = list(pageLength = 10, scrollX = TRUE), rownames = FALSE)
  })
  ...
  output$region_preview <- renderDT({
    ...
    datatable(r, filter = "top", options = list(pageLength = 10, scrollX = TRUE), rownames = FALSE)
  })
  ...
  output$exp_v1 <- renderDT({
    datatable(exp_sets$v1 %||% data.frame(Message = "Click Load ExposureSet after selecting Exposure Database."),
              filter = "top", options = list(pageLength = 10, scrollX = TRUE), rownames = FALSE)
  })
