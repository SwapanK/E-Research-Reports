  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    output$model_columns_ui <- shiny::renderUI({
      ...
      if (identical(n_models, "1")) {
        shiny::tagList(
          shiny::textInput(
            inputId = ns("model1_col"),
            label   = "Model-1 AAL column name",
            value   = m1_default,
            width   = "100%"
          )
        )
        ...
        shiny::tagList(
          ...
          shiny::textInput(
            inputId = ns("model2_col"),
            label   = "Model-2 AAL column name",
            value   = m2_default,
            width   = "100%"
          )
        )
    ...
    # -------------------------------------------------------------------------
    # Static ggplots
    # Only (re)built when the user clicks "Create Plot" in section 3.
    # -------------------------------------------------------------------------
    static_plots <- shiny::eventReactive(input$create_plot, {
      st <- obj_state()
      shiny::req(isTRUE(st$ok), st$obj)
      build_all_static_gplots(st$obj, side_by_side = FALSE)
    })
          ...
          for (nm in names(plot_list)) {
            local({
              ...
              output[[paste0(prefix, "_plot_frame_", key)]] <- shiny::renderUI({
                ...
                shiny::plotOutput(ns(paste0(prefix, "_plot_", key)), height = paste0(round(h_in * 96), "px"))
              })
              ...
              output[[paste0(prefix, "_plot_", key)]] <- shiny::renderPlot({
                ...
                override_vals <- list(
                  ...
                  tile_border_lwd = input[[paste0(prefix, "_tile_border_lwd_", key)]] %||% def$tile_border_lwd %||% 0.22,
                  show_labels = input[[paste0(prefix, "_show_labels_", key)]] %||% def$show_labels %||% TRUE
                )
                
                p <- vul_apply_overrides(base_plot, input, prefix, key, override_vals)
                p
              }, res = 96,
                 bg = (input[[paste0(prefix, "_bg_", key)]] %||% defaults[[prefix]]()$bg %||% "white"))
        ...
        local({
          ...
          shiny::observeEvent(input[[paste0(grp, "_apply_all")]], {
            def <- list(
              ...
              tile_border_lwd = input[[paste0(grp, "_default_tile_border_lwd")]] %||% 0.22,
            ...
            for (key in plot_names) {
              ...
              shiny::updateNumericInput(session, paste0(grp, "_tile_border_lwd_", key), value = def$tile_border_lwd)
    ...
    output$region_gallery_controls <- shiny::renderUI({
      ...
      vul_gallery_controls_ui(
        ...
        default_tile_border_lwd = def$tile_border_lwd, default_show_labels = def$show_labels,
      ...
      shiny::tagList(
        ...
        lapply(rnames, function(nm) {
          ...
          vul_plot_card_gallery(
            ...
            default_border_col = def$border_col, default_tile_border_lwd = def$tile_border_lwd,
            default_show_labels = def$show_labels, default_axis_text_margin_t = def$axis_text_margin_t,
            default_axis_text_vjust = def$axis_text_vjust, has_facets = TRUE
          )
        })
      )
    })
      ...
      shiny::tagList(
        ...
        lapply(pnames, function(nm) {
          ...
          vul_plot_card_gallery(
            ...
            default_border_col = def$border_col, default_tile_border_lwd = def$tile_border_lwd,
            default_show_labels = def$show_labels, default_axis_text_margin_t = def$axis_text_margin_t,
            default_axis_text_vjust = def$axis_text_vjust, has_facets = FALSE
          )
        })
      )
    })
        ...
        for (group in c("region", "state", "pct")) {
          ...
          for (key in names(plot_list)) {
            ...
            override_vals <- list(
              ...
              tile_border_lwd = input[[paste0(group, "_tile_border_lwd_", key)]] %||% def$tile_border_lwd %||% 0.22,
              show_labels = input[[paste0(group, "_show_labels_", key)]] %||% def$show_labels %||% TRUE
            )
