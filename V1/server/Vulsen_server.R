...
# VulSen server – full module with overrides, gallery, downloads, and diagnostics
# =============================================================================
...
Vulsen_server <- function(id) {
  shiny::moduleServer(id, function(input, output, session) {
          ...
          tryCatch(
            {
              ...
              message("Unique Classification values: ", paste(unique(df$Classification), collapse = ", "))
              # --------------------
              list(
                ok   = TRUE,
      ...
      tryCatch(
        {
          ...
          # ---- DIAGNOSTIC ----
          message("--- obj_state ---")
          message("region_split names: ", paste(names(o$region_split), collapse = ", "))
          message("state_split names: ", paste(names(o$state_split), collapse = ", "))
          message("model_title: ", o$model_title)
          # --------------------
    ...
    static_plots <- shiny::reactive({
      ...
      plots <- build_all_static_gplots(st$obj, side_by_side = isTRUE(input$side_by_side))
      # ---- DIAGNOSTIC ----
      message("--- static_plots ---")
      message("region plots: ", paste(names(plots$region), collapse = ", "))
      message("state plots: ", paste(names(plots$state), collapse = ", "))
      message("pct plots: ", paste(names(plots$pct), collapse = ", "))
      # --------------------
      plots
    })
    ...
    shiny::observe({
      ...
      # Create default bundles for each group
      for (group in c("region", "state", "pct")) {
        ...
        if (is.null(gp) || length(gp) == 0) {
          message("No plots for group: ", group)
          next
        }
        def <- defaults[[group]]()
        bundles <- list()
      ...
      for (group in c("region", "state", "pct")) {
        local({
          ...
          if (is.null(plot_list) || length(plot_list) == 0) {
            message("Skipping plot creation for group: ", grp, " (no plots)")
            return()
          }
          ...
          for (nm in names(plot_list)) {
            local({
              key <- nm
              prefix <- grp
              message("Creating outputs for: ", prefix, " - ", key)
              
              # Create plot frame and render plot
              ...
              # Download handler for this plot
        ...
        local({
          ...
          shiny::observeEvent(input[[paste0(grp, "_apply_all")]], {
            ...
            for (key in plot_names) {
              ...
              shiny::updateColourInput(session, paste0(grp, "_axis_line_col_", key), value = def$axis_line_col)
              shiny::updateColourInput(session, paste0(grp, "_panel_fill_", key), value = def$panel_fill)
              shiny::updateColourInput(session, paste0(grp, "_grid_col_", key), value = def$grid_col)
              ...
              shiny::updateColourInput(session, paste0(grp, "_panel_border_col_", key), value = def$border_col)
              shiny::updateNumericInput(session, paste0(grp, "_panel_border_lwd_", key), value = def$border_lwd)
              shiny::updateColourInput(session, paste0(grp, "_col_sfd_", key), value = def$col_sfd)
              shiny::updateColourInput(session, paste0(grp, "_col_com_", key), value = def$col_com)
              shiny::updateColourInput(session, paste0(grp, "_col_pen_", key), value = def$col_pen)
              shiny::updateColourInput(session, paste0(grp, "_col_cred_", key), value = def$col_cred)
    ...
    # -------------------------------------------------------------------------
    # RENDER THE PLOT UI (with fallback message)
    # -------------------------------------------------------------------------
    output$plots_ui <- shiny::renderUI({
      ...
      # ---- DIAGNOSTIC ----
      message("--- plots_ui ---")
      message("region names in p: ", paste(names(p$region), collapse = ", "))
      message("state names in p: ", paste(names(p$state), collapse = ", "))
      message("pct names in p: ", paste(names(p$pct), collapse = ", "))
      # --------------------
      ...
      # ---- DIAGNOSTIC ----
      message("rnames: ", paste(rnames, collapse = ", "))
      message("snames: ", paste(snames, collapse = ", "))
      message("pnames: ", paste(pnames, collapse = ", "))
      # --------------------
      
      # If no plots at all, show a message
      if (length(rnames) == 0 && length(snames) == 0 && length(pnames) == 0) {
        return(
          div(
            class = "info-panel",
            style = "margin-top: 20px; padding: 30px; text-align: center;",
            h4("No plots could be generated"),
        ...
        if (length(rnames) > 0) {
          tagList(
            ...
            lapply(rnames, function(nm) {
              dims <- plot_dims(o$region_split[[nm]], "region")
              plot_card(
                output_id = paste0("region_", make.names(nm)),
                title = nm,
                height = dims$height,
                ns = session$ns
              )
        ...
        if (length(snames) > 0) {
          tagList(
            ...
            lapply(snames, function(nm) {
              dims <- plot_dims(o$state_split[[nm]], "state")
              plot_card(
                output_id = paste0("state_", make.names(nm)),
                title = nm,
                height = dims$height,
                ns = session$ns
              )
        ...
        if (length(pnames) > 0) {
          tagList(
            ...
            lapply(pnames, function(nm) {
              dims <- plot_dims(p$pct_data[[nm]], "pct")
              plot_card(
                output_id = paste0("pct_", make.names(nm)),
                title = nm,
                height = dims$height,
                ns = session$ns
              )
