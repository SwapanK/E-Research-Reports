...
Vulsen_server <- function(id) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    rv <- shiny::reactiveValues(
      raw_data = NULL,          # raw uploaded/sample data
      obj = NULL,               # analysis object from build_rmd_objects()
      plots = NULL,             # static plots from build_all_static_gplots()
      bundles = NULL,           # list of plot bundles (with overrides)
      model_loaded = FALSE,
      data_loaded = FALSE,
    ...
    output$progress_panel <- shiny::renderUI({
      stages <- list(
        ...
        list(label = "Download", done = rv$plots_generated)   # 4th stage
      )
    ...
    shiny::observe({
      ...
      for (group in c("region", "state", "pct")) {
        if (group == "pct") {
          # For pct, use the pct_data list
          gp <- plots[["pct"]]
        } else {
          gp <- plots[[group]]
        }
        if (is.null(gp) || length(gp) == 0) next
        # Create bundles with default overrides
        def <- defaults[[group]]()
        bundles <- list()
        for (nm in names(gp)) {
          bundles[[nm]] <- list(
          ...
          for (nm in names(plot_list)) {
            local({
              ...
              output[[paste0(prefix, "_plot_", key)]] <- plotly::renderPlotly({
                # Get the base plot from rv$plots
                ...
                def <- defaults[[prefix]]()
                # Build the list of override values (fallback to defaults)
                override_vals <- list(
              ...
              output[[paste0(prefix, "_dl_", key)]] <- shiny::downloadHandler(
                ...
                content = function(file) {
                  # Rebuild the plot with current overrides (same as above)
                  base_plot <- if (prefix == "pct") {
                    rv$plots[["pct"]][[key]]
                  } else {
                    rv$plots[[prefix]][[key]]
      ...
      for (group in c("region", "state", "pct")) {
        local({
          ...
          shiny::observeEvent(input[[paste0(grp, "_apply_all")]], {
            # Get default values from the gallery controls
            def <- list(
            ...
            # Update the defaults reactive for this group
            defaults[[grp]](def)
            
            # Update all individual inputs for this group
    ...
    # -------------------------------------------------------------------------
    # RENDER THE PLOT UI (the missing piece)
    # -------------------------------------------------------------------------
    ...
    output$download_selected <- shiny::downloadHandler(
      ...
      content = function(file) {
        ...
        if (choice == "html") {
          # Use the fallback or the template-based report
        ...
        # For plots or everything, create a zip
        ...
        if (choice == "all") {
          # Also add processed data and HTML report
