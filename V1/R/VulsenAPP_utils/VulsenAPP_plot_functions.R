...
vul_apply_overrides <- function(p, input, prefix, key, defaults) {
  ...
  p <- p + theme(
    axis.text.x = element_text(
      size = axis_text,
      angle = axis_angle,
      hjust = if (isTRUE(axis_angle == 0)) 0.5 else 1,
      vjust = axis_text_vjust,
      margin = if (identical(prefix, "pct")) {
        margin(t = axis_text_margin_t)
      } else {
        margin(t = axis_text_margin_t, r = 20, b = 20, l = 5)
      }
    ),
  ...
  ggplot2::ggplot(
    ...
    ggplot2::theme(
      ...
      axis.text.y = ggplot2::element_text(
        size = sz$axis_y,
        face = "bold",
        margin = ggplot2::margin(t = 5, r = 10, b = 10, l = 5)
      ),
...
build_all_static_gplots <- function(obj, side_by_side = FALSE) {
  ...
  # Percentage Change only makes sense when there are two model versions
