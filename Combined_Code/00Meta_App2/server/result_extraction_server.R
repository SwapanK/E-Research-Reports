# Server module for Result Extraction App
...
resultExtractServer <- function(input, output, session) {
  ...
  session$onSessionEnded(function() {
    for (id in c("v1", "v2")) {
      if (!is.null(conns[[id]])) try(odbcClose(conns[[id]]), silent = TRUE)
    }
  })
}