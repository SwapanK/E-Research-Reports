...
vul_apply_overrides <- function(p, input, prefix, key, defaults) {
  ...
  # ---- Legend key sizing: force all bins to fit in a single row ----
  # Use small, fixed key dimensions and cap the legend text size so that
  # all 9 bins (including unused ones) always appear with colour swatches.
  # The user can still adjust legend_key_size, but we cap it at 0.35 cm
  # to guarantee the row fits; this ensures swatches are never clipped.
  fit_key_size <- min(legend_key_size, 0.35)
  # Also cap legend text to a maximum of 7 pt to keep the row narrow.
  fit_legend_text <- min(legend_text, 7)
