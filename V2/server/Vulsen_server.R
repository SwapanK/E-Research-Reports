    ...
    setup_legend_manager <- function(tag, scheme_staging, committed_scheme, default_bins, scheme_type, value_source) {
      ...
      shiny::observeEvent(input[[paste0(tag, "_colour_apply")]], {
        ...
        if (isTRUE(v$ok)) {
          ...
          session$sendCustomMessage("show-toast", list(
            text = "Colour updated (apply to plots to see changes)",
            type = "info",
            icon = "fa-palette",
            duration = 2500
          ))
      ...
      shiny::observeEvent(input[[paste0(tag, "_legend_table_cell_edit")]], {
        ...
        if (isTRUE(v$ok)) {
          ...
          legend_msg[[tag]](NULL)
          session$sendCustomMessage("show-toast", list(
            text = "Legend updated (apply to plots to see changes)",
            type = "info",
            icon = "fa-edit",
            duration = 2000
          ))
      ...
      shiny::observeEvent(input[[paste0(tag, "_legend_load_default")]], {
        ...
        legend_msg[[tag]](NULL)
        session$sendCustomMessage("show-toast", list(
          text = "Default legend loaded (apply to plots to see changes)",
          type = "info",
          icon = "fa-rotate-left",
          duration = 2000
        ))
      ...
      shiny::observeEvent(input[[paste0(tag, "_legend_json_file")]], {
        ...
        legend_msg[[tag]](NULL)
        session$sendCustomMessage("show-toast", list(
          text = "Legend JSON loaded (apply to plots to see changes)",
          type = "info",
          icon = "fa-file-import",
          duration = 2000
        ))
      ...
      shiny::observeEvent(input[[paste0(tag, "_legend_create_bins")]], {
        ...
        legend_msg[[tag]](NULL)
        session$sendCustomMessage("show-toast", list(
          text = paste("Created", nrow(res), "quantile bins (apply to plots to see changes)"),
          type = "info",
      ...
      local({
        ...
        shiny::observeEvent(input[[paste0(grp, "_apply_all")]], {
          ...
          if (grp %in% c("region", "state")) {
            ...
            session$sendCustomMessage("show-toast", list(
              text = "Relative AAL legend applied to plots",
              type = "success",
              icon = "fa-check",
              duration = 2000
            ))
            ...
            session$sendCustomMessage("show-toast", list(
              text = "Percentage Change legend applied to plots",
              type = "success",
              icon = "fa-check",
              duration = 2000
            ))
