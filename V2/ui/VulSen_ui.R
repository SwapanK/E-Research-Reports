...
VulSen_ui <- function(id) {
  ...
  shiny::tagList(
    shiny::tags$head(
      ...
      # Loaded AFTER dashboard_style.css on purpose: it scopes an override
      # to `.vulsen-app-shell` only, so the rest of the app (which also
      # loads dashboard_style.css) is unaffected. See vulsen_style.css.
