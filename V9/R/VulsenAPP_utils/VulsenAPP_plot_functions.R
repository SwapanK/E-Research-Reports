...
#
# CHANGE LOG (this pass - legend fixes):
#   Fix A - "Y-axis label margin right (px)" override removed entirely
#           (control retired; axis.text.y now uses a fixed margin of 2pt).
#   Fix B - Legend bins that have no data in them no longer get clipped
#           off the single-row legend. We now use fixed small key
#           dimensions (0.3 cm) and cap legend text size to ensure that
#           all 9 bins always fit on one row without clipping the colour
#           swatches. The user can still control legend_key_size, but it
#           is capped at 0.35 to guarantee fit.
#   Fix C - The legend row is anchored to the left edge of the full plot
#           using legend.justification/legend.box.just = "left".
# =============================================================================
...
vul_apply_overrides <- function(p, input, prefix, key, defaults) {
  ...
  # ---- Legend key sizing ----
  # Previously this block silently capped legend_key_size at 0.35cm and
  # legend_text at 7pt (via fit_key_size / fit_legend_text below), no matter
  # what the user set in the "Legend Key" / "Legend Text" controllers. That
  # is why those two controls appeared to do nothing - the raw values were
  # read correctly but then overwritten by the cap before being applied to
  # the theme. The controllers now drive the plot directly.
  #
  # NOTE: the original cap existed to guarantee all 9 legend bins fit on a
  # single row without clipping (see "Fix B" in the changelog above). If
  # users push Legend Text / Legend Key size high enough with a full 9-bin
  # legend, the row may wrap or clip again - if that resurfaces, consider
  # switching legend.position to "bottom" with guide_legend(nrow = ...)
  # instead of silently re-capping the values.
  ...
  p <- p + theme(
    ...
    # Fix A: Y-axis label margin is now fixed at 2pt; UI control removed.
    axis.text.y  = element_text(size = axis_text, margin = margin(r = 2, unit = "pt")),
    # Problem 5: axis titles are never shown.
    axis.title.x = element_blank(),
    axis.title.y = element_blank(),
    plot.title   = element_text(size = plot_title),
    # Problem 6: strip font face is fixed to bold.
    strip.text   = element_text(size = strip_text, face = "bold"),
    # Fix B & C: legend text capped, key dimensions fixed small, left-aligned.
    ...
    # Model Panel Gap (px) - actual gap between model facets.
  ...
  # (Problem 8: font face is fixed to bold).
...
# scheme. Always the FULL scheme - no per-plot trimming.
...
plot_rel_gg <- function(
  ...
  ggplot2::ggplot(
    ...
    ggplot2::theme(
      ...
      panel.border = ggplot2::element_rect(color = "black", fill = NA, linewidth = 0.3),  # thinner border
      legend.position = "top",
      ...
      # Y-axis label margin is now applied in vul_apply_overrides() with a fixed 2pt.
