...
inputPrepServer <- function(input, output, session) {
  ...
  # Start with sample files enabled
  ...
  output$progress_panel <- renderUI({
    ...
    settings_ok <- isTruthy(input$vendor) && isTruthy(input$module) && isTruthy(input$version)
    exposure_ok <- isTruthy(input$country) && isTruthy(input$peril) && isTruthy(input$subperil) && isTruthy(input$suffix)
    data_ok <- !is.null(location_data()) && !is.null(combination_data())
    generate_ok <- !is.null(generated())
    download_ok <- generate_ok
    
    stages <- list(
      list(label = "Settings", done = settings_ok),
      list(label = "Exposure", done = exposure_ok),
      list(label = "Data", done = data_ok),
      list(label = "Generate", done = generate_ok),
      list(label = "Download", done = download_ok)
    )
