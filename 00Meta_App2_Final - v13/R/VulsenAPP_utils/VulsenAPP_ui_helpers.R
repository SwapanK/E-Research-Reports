...
vul_class_reorder_ui <- function(ns, key, prefix, safe_key) {
  ...
  sec_control_section(
    ...
    div(
      ...
      sec_hint(paste0(
        "Reorder or rename the \u201c", key, "\u201d classes on this plot's x-axis ",
        "in a popup, preview the result, then click Apply to update the plot."
      )),
...
#' order/rename state.
#'
#' Everything inside the modal - the Order/Name/Rename table, the arrow
#' reordering, the Rename text inputs, and the "A | L | R | ..." preview -
#' is plain HTML built and re-rendered entirely by the JS below, against a
#' plain in-memory array. Nothing touches Shiny until Apply is clicked. The
#' modal's DOM is created fresh on every open and torn down by
#' removeModal()/Cancel, so the click/input listeners are bound directly to
#' this modal's own elements - no document-level delegation, no
#' "already-bound" guard needed (unlike the old v1 arrow-click script).
#'
...
#' @param order_df Current committed order/name/rename data.frame for this
#'   card (class_order_committed[[ck]]) - what the modal is seeded with.
vul_class_order_modal_ui <- function(ns, key, apply_input_id, order_df) {
  ...
  seed_json <- jsonlite::toJSON(
    lapply(seq_len(nrow(df)), function(i) list(name = df$name[i], rename = df$rename[i])),
    auto_unbox = TRUE
  )
  ...
  shiny::modalDialog(
    ...
    sec_hint(paste0(
      "Reorder or rename the \u201c", key, "\u201d classes on this plot's x-axis. ",
      "Use the arrows to reorder, edit Rename to relabel, watch the preview update, then click Apply."
    )),
    ...
    tags$script(HTML(sprintf(
      "(function() {
         ...
         function renderPreview() {
           var chips = state.map(function(row) {
             ...
             return '<span class=\"vul-order-preview-chip\">' + escapeHtml(label) + '</span>';
           });
         ...
         function renderRows() {
           ...
           state.forEach(function(row, i) {
             html += '<tr>' +
               ...
               '<td style=\"cursor:text;\"><input type=\"text\" class=\"form-control vul-order-rename-input\" data-idx=\"' + i + '\" value=\"' + escapeAttr(row.rename) + '\"></td>' +
             '</tr>';
           });
         ...
         tbody.addEventListener('click', function(e) {
           var btn = e.target.closest('.vul-order-arrow');
           if (!btn) return;
         ...
         applyBtn.addEventListener('click', function() {
           ...
           var payload = state.map(function(row, i) {
             ...
             return { order: i + 1, name: row.name, rename: rn };
           });
      ...
      seed_id, tbody_id, preview_id, apply_btn_id, apply_target
    )))
  )
}
