...
#     order/name/rename (same shape as default_class_order_state() in
#     VulsenAPP_config.R). When supplied, it's used to rebuild
#     scale_x_discrete() with `limits` set to the reordered `name` values
#     and `labels` set to their `rename` values, so the x-axis both
#     re-sorts and re-labels the class ticks. `name` values are matched
#     against the plot's actual Description factor levels by name (not
#     position), so class_order rows that don't exist in this particular
#     plot's data are simply ignored rather than erroring.
...
#'   (same shape as default_class_order_state() in VulsenAPP_config.R) -
#'   the COMMITTED per-plot X-Axis Class Order & Labels state for this
#'   card. NULL/zero-row means "no reorder/rename" and the plot's existing
#'   scale_x_discrete() (set inside plot_rel_gg()/plot_pct_gg()) is left
#'   untouched.
vul_apply_overrides <- function(p, input, prefix, key, defaults, class_order = NULL) {
  ...
  # ---- X-Axis Class Order & Labels (reorder/rename) ----
  # `class_order` is the COMMITTED order/name/rename data.frame for this
  # card (staging happens server-side; only Apply commits it here). We
  ...
  #
  # `limits` reorders the axis; `labels` is a NAME-KEYED vector (original
  # class code -> user-typed Rename text), which is matched by value, not
  # position, so it's safe even if class_order carries rows for classes
  # that aren't present on this particular plot (e.g. a class missing
  # from one region's data) - those rows are simply dropped below.
  if (!is.null(class_order) && is.data.frame(class_order) && nrow(class_order) > 0) {
    co <- class_order[order(class_order$order), c("name", "rename"), drop = FALSE]
