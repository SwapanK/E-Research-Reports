...
#   - vul_apply_overrides() gains a `class_order` param (per-plot X-Axis
#     Class Order & Labels feature): a committed data.frame with columns
#     order/name/rename/visible (same shape as default_class_order_state()
#     in VulsenAPP_config.R). When supplied, it's used to rebuild
#     scale_x_discrete() with `limits` set to the reordered, visible-only
#     `name` values and `labels` set to their `rename` values, so the
#     x-axis re-sorts, re-labels, AND hides any class marked
#     visible == FALSE. `name` values are matched against the plot's
#     actual Description factor levels by name (not position), so
#     class_order rows that don't exist in this particular plot's data
#     are simply ignored rather than erroring.
#   - axis.text.x and axis.text.y are now both explicitly bold in
#     vul_apply_overrides() (axis.text.x already was; axis.text.y now
#     matches it - previously the override theme() call silently dropped
#     the bold face that plot_rel_gg()/plot_pct_gg() set on the y-axis,
#     since a later theme(axis.text.y = element_text(...)) replaces the
#     element wholesale rather than merging with it).
#   - Added per-class Show/Hide (`visible` column). A class with
#     visible == FALSE is dropped from scale_x_discrete()'s `limits`
#     before the scale is built, which removes its tiles/labels/tick from
#     the rendered heatmap without touching the underlying data. Missing
#     `visible` column (older committed states) or NA rows default to
#     visible == TRUE, so this is backward compatible with states saved
#     before the hide feature existed.
# =============================================================================

# ---- Apply overrides to a ggplot ----
#' @param class_order Optional data.frame with columns order/name/rename/
#'   visible (same shape as default_class_order_state() in
#'   VulsenAPP_config.R) - the COMMITTED per-plot X-Axis Class Order &
#'   Labels state for this card. NULL/zero-row means "no reorder/rename/
#'   hide" and the plot's existing scale_x_discrete() (set inside
#'   plot_rel_gg()/plot_pct_gg()) is left untouched. Rows with
#'   visible == FALSE are excluded from the rebuilt axis; a missing
#'   `visible` column is treated as all-visible for backward compatibility.
vul_apply_overrides <- function(p, input, prefix, key, defaults, class_order = NULL) {
  ...
  p <- p + theme(
    ...
    axis.text.y  = element_text(size = axis_text, margin = margin(r = 2, unit = "pt"), face = "bold"),
  ...
  # ---- X-Axis Class Order & Labels (reorder/rename/hide) ----
  # `class_order` is the COMMITTED order/name/rename/visible data.frame for
  # this card (staging happens server-side; only Apply commits it here). We
  # rebuild scale_x_discrete() rather than just adding limits/labels on
  # top, because ggplot2 replaces (not merges) a duplicate discrete scale
  # - so expand/guide have to be restated here or they'd silently revert
  # to ggplot's defaults.
  #
  # `limits` reorders (and, by omission, hides) the axis; `labels` is a
  # NAME-KEYED vector (original class code -> user-typed Rename text),
  # which is matched by value, not position, so it's safe even if
  # class_order carries rows for classes that aren't present on this
  # particular plot (e.g. a class missing from one region's data) - those
  # rows are simply dropped below.
  #
  # `visible` (TRUE/FALSE per class) is the Show/Hide toggle: a class with
  # visible == FALSE is excluded from `limits`, so ggplot drops its tiles/
  # text/axis-tick entirely - the same "just don't include it in limits"
  # mechanism reordering already relied on. `visible` may be absent on
  # older committed states (pre-hide-feature) or NA on an individual row;
  # both are treated as visible so this stays backward compatible.
  if (!is.null(class_order) && is.data.frame(class_order) && nrow(class_order) > 0) {
    keep_cols <- intersect(c("name", "rename", "visible"), names(class_order))
    co <- class_order[order(class_order$order), keep_cols, drop = FALSE]
    co <- co[!is.na(co$name) & nzchar(co$name), , drop = FALSE]
    if (!("visible" %in% names(co))) co$visible <- TRUE
    co$visible[is.na(co$visible)] <- TRUE
    co <- co[co$visible, , drop = FALSE]
    if (nrow(co) > 0) {
      label_map <- stats::setNames(co$rename, co$name)
