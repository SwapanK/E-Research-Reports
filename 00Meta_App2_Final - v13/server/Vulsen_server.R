  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    setup_class_order_manager <- function(prefix, key, safe_key) {
      ...
      shiny::observeEvent(input[[apply_id]], {
        ...
        # same class set, same row count, no blank renames - before
        # trusting it as the new committed state.
    ...
    shiny::observeEvent(input$peril, {
      ...
      if (!is.null(choices)) {
        default_selected <- if ("AllPeril" %in% choices) "AllPeril" else choices[1]
        shiny::updateSelectInput(session, "subperil", choices = choices, selected = default_selected)
      }
