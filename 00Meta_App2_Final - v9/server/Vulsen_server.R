...
Vulsen_server <- function(id, cart, username) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    # Fixed on-screen rendering resolution (px per inch). This is
    # deliberately separate from the user-controlled export DPI - Plot
    # Width (in) / Plot Height (in) define the true figure size, and this
    # constant is only used to translate those inches into a plotOutput()
    # pixel box for the browser. Export (ggsave) uses the user's DPI
    # setting for resolution while keeping the same width/height in inches
    # (Problem 3 - single source of truth for on-screen + exported sizing).
    ...
    shiny::observe({
          ...
          for (nm in names(plot_list)) {
            local({
              ...
              output[[paste0(prefix, "_plot_frame_", key)]] <- shiny::renderUI({
                def <- defaults[[prefix]]()
                width_in  <- input[[paste0(prefix, "_width_in_", key)]]  %||% def$width_in  %||% 9
                height_in <- input[[paste0(prefix, "_height_in_", key)]] %||% def$height_in %||% 5
                width_px  <- max(200, round(width_in  * VULSEN_SCREEN_DPI))
                height_px <- max(150, round(height_in * VULSEN_SCREEN_DPI))
                shiny::plotOutput(
                  ns(paste0(prefix, "_plot_", key)),
      ...
      # This fixes the issue where Data Label Colour pickers appear as plain text boxes.
    ...
    # Add to cart
    # -------------------------------------------------------------------------
    # Mirrors secmod_server.R's sec_cart_click observer: rebuilds the plot
    # with vul_apply_overrides() using that card's *current* input values -
    # the exact same function/inputs the on-screen renderPlot() and the
    # download handler use - so what lands in the cart is pixel-identical
    # to what's on screen, with every customisation (size, colours, text,
    # legend, labels) preserved.
      ...
      local({
        ...
        shiny::observeEvent(input[[paste0(grp, "_apply_all")]], {
          ...
          for (key in plot_names) {
            shiny::updateNumericInput(session, paste0(grp, "_width_in_", key), value = def$width_in)
            shiny::updateNumericInput(session, paste0(grp, "_height_in_", key), value = def$height_in)
            shiny::updateNumericInput(session, paste0(grp, "_dpi_", key), value = def$dpi)
            shiny::updateCheckboxInput(session, paste0(grp, "_transparent_bg_", key), value = def$transparent_bg)
            shiny::updateNumericInput(session, paste0(grp, "_panel_gap_px_", key), value = def$panel_gap_px)
            shiny::updateNumericInput(session, paste0(grp, "_axis_text_", key), value = def$axis_text)
            shiny::updateSelectInput(session, paste0(grp, "_x_rotation_", key), selected = def$x_rotation)
            shiny::updateNumericInput(session, paste0(grp, "_x_vjust_", key), value = def$x_vjust)
          ...
          # ---- Force a re-sync of colour swatches after updating all inputs ----
          session$sendCustomMessage("sync-colour-swatches", list())
