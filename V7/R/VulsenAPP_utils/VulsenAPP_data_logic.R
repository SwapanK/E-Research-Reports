...
#   Problem 2 - Legend defaults updated to text=8, title=7, key=0.5.
#   Problem 3 - `y_axis_margin_r` removed (controller retired; margin is
#               now fixed at 2pt inside vul_apply_overrides()).
...
vul_default_overrides <- function(group = c("region", "state", "pct")) {
  ...
  # Uniform legend defaults (Problem 2)
  by_group <- switch(
    group,
  ...
  c(
    list(
      ...
      # NOTE: this key must be named `panel_gap_px` because that's what
      # vul_apply_overrides() in VulsenAPP_plot_functions.R actually reads
      # (defaults$panel_gap_px). It was previously named `gap_px` here,
      # which meant this default silently never applied and the plotting
      # function's own hardcoded fallback (16) was used instead.
      panel_gap_px = 8,
    ...
    by_group,
    # Legend text/title sizes are uniform across all groups (Problem 2)
