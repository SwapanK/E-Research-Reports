...
# ---- Required base columns (without model AAL columns) ----
...
get_expected_headers <- function(model_family, model1_col, model2_col = NULL) {
  ...
  model_cols <- c(trimws(model1_col))
  if (!is.null(model2_col) && nzchar(trimws(model2_col))) {
    model_cols <- c(model_cols, trimws(model2_col))
  }
  unique(c(base, model_cols))
}

# ---- Validate that uploaded CSV headers exactly match expected ----
validate_csv_headers <- function(df, model_family, model1_col, model2_col = NULL) {
  actual <- trimws(names(df))
  expected <- get_expected_headers(model_family, model1_col, model2_col)
  missing <- setdiff(expected, actual)
  extra   <- setdiff(actual, expected)
  
  if (length(missing) == 0 && length(extra) == 0) {
    return(list(ok = TRUE, msg = "Headers match exactly."))
  } else {
    msg <- paste0(
...
# ---- Validate uploaded data (legacy, kept for backward compatibility) ----
