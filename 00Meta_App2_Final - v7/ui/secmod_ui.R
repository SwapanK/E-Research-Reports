...
sec_plot_card_gallery <- function(ns, key, label, prefix,
  ...
  div(
    ...
    div(
      ...
      # "LOB text size" (facet strip size) only matters for Individual Modifier
      # (s6), which facets by STATECODE. State Sensitivity (s5) plots don't
      # facet, so the control is dropped there.
      sec_control_section(
      ...
      # Neither gallery plot currently has a data-label layer (only Credit/
      # Penalty, stage 4, does) — hidden for s5 per current request; left in
      # place for s6 pending a decision on whether to remove it there too.
      if (prefix != "s5") {
        sec_control_section(
