...
apply_plot_overrides <- function(p, input, prefix, key, defaults, is_credit = FALSE) {
  ...
  axis_text_hjust <- (input[[paste0(prefix, "_axis_text_hjust_", key)]] %||% defaults$axis_text_hjust) %||% 1
  axis_wrap_width <- (input[[paste0(prefix, "_axis_wrap_width_", key)]] %||% defaults$axis_wrap_width) %||% 30
  ...
  p <- p + theme(
    axis.text.x = element_text(size = axis_text, angle = axis_angle,
                               hjust = axis_text_hjust, vjust = axis_text_vjust,
  ...
  # ---- X-axis tick label wrapping (independent control from any data-label
  #      wrap width) - wraps the rendered tick text only, never the underlying
  #      data, so it is safe to apply regardless of which variable is mapped
  #      to x for this particular plot (Modifier / Description / etc). ----
  if (!is.null(axis_wrap_width) && is.numeric(axis_wrap_width) && axis_wrap_width > 0) {
    # function_Secmod.R's plot functions already add their own
    # scale_x_discrete(labels=...); re-applying it here (so the LIVE UI
    # wrap-width control works) legitimately replaces that first scale,
    # which ggplot2 reports as an informational message, not an error.
    suppressMessages({
      p <- p + scale_x_discrete(labels = function(x) str_wrap(x, width = axis_wrap_width))
    })
  }
...
secmod_server <- function(input, output, session, cart, username) {
  ...
  gallery_defaults_s5 <- reactiveVal(
    list(
      ...
      axis_text_margin_t = 5, axis_text_vjust = 1, axis_text_hjust = 1,
      axis_wrap_width = 30,
      bg = FALSE
    )
  )
  ...
  # ---- Sample file downloads (vendor-dependent via secmod_sample_paths()) ----
  output$sec_dl_sample_state <- downloadHandler(
    filename = function() basename(secmod_sample_paths(input$sec_vendor %||% "Moody's")$state),
    content = function(file) {
      sp <- secmod_sample_paths(input$sec_vendor %||% "Moody's")
      file.copy(sp$state, file, overwrite = TRUE)
    }
  )
  output$sec_dl_sample_country <- downloadHandler(
    filename = function() basename(secmod_sample_paths(input$sec_vendor %||% "Moody's")$usa),
    content = function(file) {
      sp <- secmod_sample_paths(input$sec_vendor %||% "Moody's")
      file.copy(sp$usa, file, overwrite = TRUE)
    }
  )
  output$sec_dl_sample_mapping <- downloadHandler(
    filename = function() basename(secmod_sample_paths(input$sec_vendor %||% "Moody's")$mapping),
    content = function(file) {
      sp <- secmod_sample_paths(input$sec_vendor %||% "Moody's")
      file.copy(sp$mapping, file, overwrite = TRUE)
    }
  )
  
  # ---- Helper: read a CSV upload (CSV-only; .rds no longer supported) ----
  read_any <- function(datapath, original_name) {
    ext <- tolower(tools::file_ext(original_name))
    if (ext == "csv") utils::read.csv(datapath, stringsAsFactors = FALSE)
    else stop("Unsupported file type: ", ext, ". Only .csv files are accepted.")
  }
  ...
  # ---- Helper: find the first numeric-looking column (left to right) ----
  # A column counts as "numeric" if it's already numeric/integer, or if its
  # values parse cleanly as numbers when coerced (covers numeric columns that
  # were read in as character, e.g. due to stray whitespace or quoting).
  first_numeric_col <- function(df) {
    if (is.null(df) || ncol(df) == 0) return(NA_character_)
    for (nm in names(df)) {
      col <- df[[nm]]
      if (is.numeric(col)) return(nm)
      if (is.character(col) || is.factor(col)) {
        vals <- trimws(as.character(col))
        vals <- vals[nzchar(vals)]
        if (length(vals) > 0 && !any(is.na(suppressWarnings(as.numeric(vals))))) {
          return(nm)
        }
      }
    }
  ...
  # ---- State AAL: column-picker dropdown appears as soon as the file loads ----
  observeEvent(input$sec_file_state, {
    req(input$sec_file_state)
    
    output$sec_state_aal_col_ui <- renderUI({
      df_cols <- tryCatch(
        names(read_any(input$sec_file_state$datapath, input$sec_file_state$name)),
        error = function(e) NULL
      )
      
      if (is.null(df_cols)) {
        return(div(class = "text-danger", "Could not read columns from the uploaded State AAL file."))
      }
      
      default_col <- first_numeric_col(read_any(input$sec_file_state$datapath, input$sec_file_state$name))
      if (is.na(default_col)) default_col <- df_cols[1]
      
      selectInput(
        session$ns("sec_state_aal_col"),
        "State AAL column",
        choices = df_cols,
  ...
  # ---- Country AAL: column-picker dropdown appears as soon as the file loads ----
  observeEvent(input$sec_file_usa, {
    req(input$sec_file_usa)
    
    output$sec_country_aal_col_ui <- renderUI({
      df_cols <- tryCatch(
        names(read_any(input$sec_file_usa$datapath, input$sec_file_usa$name)),
        error = function(e) NULL
      )
      
      if (is.null(df_cols)) {
        return(div(class = "text-danger", "Could not read columns from the uploaded Country AAL file."))
      }
      
      default_col <- first_numeric_col(read_any(input$sec_file_usa$datapath, input$sec_file_usa$name))
      if (is.na(default_col)) default_col <- df_cols[1]
      
      selectInput(
        session$ns("sec_country_aal_col"),
        "Country AAL column",
        choices = df_cols,
        selected = default_col,
        width = "100%",
  ...
  observeEvent(input$sec_analyse, {
    ...
    # CSV-only enforcement (UI already restricts accept=.csv, but validate defensively)
    bad_ext <- c()
    ...
    # AAL column selection now comes from the per-file dropdowns (populated
    # as soon as each file is uploaded), rather than from the shared "Model
    # version" text field. "Model version" is kept as a free-text label used
    # only for the downloaded report's filename.
    if (!nzchar(trimws(input$sec_state_aal_col %||% ""))) {
      showNotification("Please select the AAL column for the State AAL file.", type = "error")
      return()
    }
    if (!nzchar(trimws(input$sec_country_aal_col %||% ""))) {
      showNotification("Please select the AAL column for the Country AAL file.", type = "error")
      return()
    }
    
    tryCatch({
      state_aal_col   <- input$sec_state_aal_col
      country_aal_col <- input$sec_country_aal_col
      
      # Read State AAL, then rename the selected column to "AAL"
      state_df <- read_any(input$sec_file_state$datapath, input$sec_file_state$name)
      if (!(state_aal_col %in% names(state_df))) {
        stop(sprintf("Selected State AAL column '%s' was not found in the uploaded file.", state_aal_col))
      }
      if (state_aal_col != "AAL") {
        names(state_df)[names(state_df) == state_aal_col] <- "AAL"
      }
      validate_columns(state_df, c("STATECODE", "Classification", "Description", "AAL"), "State AAL")
      
      # Read Country AAL, then rename the selected column to "AAL"
      country_df <- read_any(input$sec_file_usa$datapath, input$sec_file_usa$name)
      if (!(country_aal_col %in% names(country_df))) {
        stop(sprintf("Selected Country AAL column '%s' was not found in the uploaded file.", country_aal_col))
      }
      if (country_aal_col != "AAL") {
        names(country_df)[names(country_df) == country_aal_col] <- "AAL"
      }
  ...
  output$sec_state_aal_table <- renderDT({
    ...
    # selectize.js, which clobbers Shiny's global selectize plugin registry
    # (dropping "selectize-plugin-a11y") for the rest of the browser
    # session, breaking selectInputs rendered via renderUI elsewhere on the
    # page (e.g. Vulsen's subperil/model-column dropdowns). The 'f' in
    # dom = 'Bfrtip' already provides a global search box.
  ...
  output$sec_country_aal_table <- renderDT({
    req(rv$aal_USA)
    # filter = "top" removed -- see note in sec_state_aal_table above.
  ...
  observeEvent(input$sec_upload_final, {
    ...
    tryCatch({
      ...
      showNotification(paste("Upload failed:", conditionMessage(e)), type = "error", duration = 8)
    })
  })
  ...
  credit_penalty_plot_final <- reactive({
    ...
    p <- p + theme(
      axis.text.x = element_text(size = input$s4_axis_text %||% 12,
                                 ...
                                 hjust = input$s4_axis_text_hjust %||% 1, vjust = input$s4_axis_text_vjust %||% 1,
    ...
    # ---- X-tick (Modifier) label wrapping — separate control from the
    # Description data-label wrap width above. Wraps rendered tick text
    # only; never touches the underlying Modifier column/order. ----
    axis_wrap_width <- input$s4_axis_wrap_width %||% 30
    if (is.numeric(axis_wrap_width) && axis_wrap_width > 0) {
      # Credit_Penalty() already adds its own scale_x_discrete(); this
      # legitimately replaces it so the live UI control works.
      suppressMessages({
        p <- p + scale_x_discrete(labels = function(x) str_wrap(x, width = axis_wrap_width))
      })
    }
    ...
    if (length(p$layers) > 0) {
      # Pixel-based, line-count-independent data-label gap. The previous
      # fractional vjust (a fraction of each label's OWN wrapped-text
      # height) made the visual gap grow with the number of wrapped lines
      # in a given Description ("None" = 1 line vs "Roof mounted impact
      # resistant array" = 3 lines), which is why the padding looked
      # random from bar to bar. Converting the user's pixel input into an
      # equivalent vjust fraction PER LABEL (dividing by that label's own
      # line count) cancels the height-dependence, so every bar gets the
      # same *visual* gap regardless of how many lines its label wraps to.
      label_gap_px  <- input$s4_label_nudge_y %||% 8
      label_size_mm <- input$s4_label_size %||% 3
      dpi_val       <- input$s4_dpi %||% 300
      pt_per_mm     <- 72.27 / 25.4  # ggplot2's internal mm -> pt constant (.pt)
      line_height_px <- label_size_mm * pt_per_mm * (dpi_val / 72.27) * 1.2
      vjust_unit <- if (is.finite(line_height_px) && line_height_px > 0) {
        label_gap_px / line_height_px
      } else {
        0.2
      }
      
      has_value_desc <- !is.null(p$data) &&
        all(c("Value", "Description") %in% names(p$data))
      if (has_value_desc) {
        nlines <- pmax(1, lengths(strsplit(as.character(p$data$Description), "\n")))
        vjust_vec <- ifelse(p$data$Value >= 0,
                            -vjust_unit / nlines,
                            1 + vjust_unit / nlines)
      } else {
        vjust_vec <- NULL
      ...
      for (i in seq_along(p$layers)) {
        if (inherits(p$layers[[i]]$geom, "GeomText")) {
          ...
          if (!is.null(vjust_vec) && length(vjust_vec) == nrow(p$data)) {
            p$layers[[i]]$aes_params$vjust <- vjust_vec
          }
        }
      }
    }
  ...
  observeEvent(input$sec_credit, {
    ...
    tryCatch({
      ...
      showNotification(paste("Chart failed:", conditionMessage(e)), type = "error", duration = 8)
    })
  })
    ...
    div(
      ...
      div(
        ...
        sec_control_section(
          ...
          numericInput(session$ns("s4_axis_text_hjust"), "X label hjust", value = 1, min = 0, max = 1, step = 0.05, width = "80px"),
          numericInput(session$ns("s4_axis_wrap_width"), "X-tick wrap width", value = 30, min = 5, max = 60, step = 1, width = "80px")
        ),
        ...
        sec_control_section(
          ...
          numericInput(session$ns("s4_label_wrap_width"), "Description wrap width", value = 12, min = 5, max = 50, step = 1, width = "80px"),
          numericInput(session$ns("s4_label_nudge_y"), "Label gap (px)", value = 8, min = 0, max = 60, step = 1, width = "80px")
        )
      )
    )
  })
  ...
  observeEvent(input$sec_state_plots, {
    ...
    tryCatch({
          ...
          output[[paste0("s5_dl_", key)]] <- downloadHandler(
            ...
            content = function(file) {
              ...
              defaults <- list(
                ...
                axis_text_hjust = def$axis_text_hjust %||% 1,
                axis_wrap_width = def$axis_wrap_width %||% 30
              )
      ...
      showNotification(paste("Generation failed:", conditionMessage(e)), type = "error", duration = 8)
    })
  })
  ...
  output$sec_stage5_gallery_controls <- renderUI({
    ...
    sec_gallery_controls_ui(session$ns, "s5", length(rv$state_lob_plots),
                            ...
                            default_axis_text_hjust = def$axis_text_hjust %||% 1,
                            default_axis_wrap_width = def$axis_wrap_width %||% 30)
  })
  ...
  output$sec_stage5_gallery <- renderUI({
    ...
    tagList(lapply(keys, function(k) {
      sec_plot_card_gallery(
        ...
        default_axis_text_hjust = defaults$axis_text_hjust,
        default_axis_wrap_width = defaults$axis_wrap_width
      )
    }))
  })
  ...
  observeEvent(input$s5_apply_all, {
    ...
    def <- list(
      ...
      axis_text_hjust = input$s5_default_axis_text_hjust %||% 1,
      axis_wrap_width = input$s5_default_axis_wrap_width %||% 30,
    ...
    for (k in names(rv$state_lob_plots)) {
      ...
      updateNumericInput(session, paste0("s5_axis_text_hjust_", k), value = def$axis_text_hjust)
      updateNumericInput(session, paste0("s5_axis_wrap_width_", k), value = def$axis_wrap_width)
    }
  })
  ...
  observeEvent(input$s6_apply_all, {
    ...
    def <- list(
      ...
      axis_text_hjust = input$s6_default_axis_text_hjust %||% 1,
      axis_wrap_width = input$s6_default_axis_wrap_width %||% 30,
    ...
    for (k in names(rv$modifier_plots)) {
      ...
      updateNumericInput(session, paste0("s6_axis_text_hjust_", k), value = def$axis_text_hjust)
      updateNumericInput(session, paste0("s6_axis_wrap_width_", k), value = def$axis_wrap_width)
    }
  })
    ...
    tryCatch({
      ...
      for (k in names(rv$state_lob_plots)) {
        ...
        defaults <- list(
          ...
          axis_text_hjust = def5$axis_text_hjust %||% 1,
          axis_wrap_width = def5$axis_wrap_width %||% 30
        )
      ...
      for (k in names(rv$modifier_plots)) {
        ...
        defaults <- list(
          ...
          axis_text_hjust = def6$axis_text_hjust %||% 1,
          axis_wrap_width = def6$axis_wrap_width %||% 30
        )
      ...
      template_path <- "www/secmod_customized_template.Rmd"
      ...
      removeNotification(notif_id)
      showNotification(paste("Failed to generate report:", e$message), type = "error", duration = 8)
    }, finally = {
  ...
  observeEvent(input$sec_cart_click, {
    ...
    plot_obj <- switch(prefix,
                       "s4" = {
                         defaults <- list(
                           axis_text = input$s4_axis_text %||% 12,
                           axis_title = input$s4_axis_title %||% 14,
                           plot_title = input$s4_plot_title %||% 16,
                           strip_text = input$s4_strip_text %||% 12,
                           legend_text = input$s4_legend_text %||% 10,
                           axis_angle = input$s4_axis_angle %||% 20,
                           legend_show = input$s4_legend_show %||% TRUE,
                           col_sfd = NA_character_,
                           col_com = NA_character_,
                           col_pen = input$s4_col_pen %||% "#F0B323",
                           col_cred = input$s4_col_cred %||% "#6FACDE",
                           legend_key_size = input$s4_legend_key_size %||% 0.8,
                           panel_spacing = input$s4_panel_spacing %||% 0.5,
                           bg = input$s4_bg %||% FALSE,
                           axis_text_margin_t = input$s4_axis_text_margin_t %||% 5,
                           axis_text_vjust = input$s4_axis_text_vjust %||% 1,
                           axis_text_hjust = input$s4_axis_text_hjust %||% 1,
                           axis_wrap_width = input$s4_axis_wrap_width %||% 30,
                           label_nudge_y = input$s4_label_nudge_y %||% 8
                         )
                         credit_penalty_plot_final()
                       },
                       "s5" = {
    ...
    # Attach the styling/defaults active at add-to-cart time, plus dimensions,
    # so docx_generator.R can render a per-figure settings block later.
    customization <- c(defaults, list(width_in = w, height_in = h, dpi = dpi))
  ...
  observeEvent(input$sec_load_demo, {
    tryCatch({
      # Read sample files based on the selected vendor
      sp <- secmod_sample_paths(input$sec_vendor %||% "Moody's")
      state_path <- sp$state
      usa_path <- sp$usa
      mapping_path <- sp$mapping
      ...
      # Smart-detect the AAL column the same way uploaded files do: use the
      # existing "AAL" column if the sample file already has one, otherwise
      # fall back to the first numeric-looking column. Demo data is not
      # required to have a column literally named "AAL".
      if (!("AAL" %in% names(state_df))) {
        state_aal_col <- first_numeric_col(state_df)
        if (is.na(state_aal_col)) {
          stop("Could not detect a numeric AAL column in the sample State AAL file.")
        }
        names(state_df)[names(state_df) == state_aal_col] <- "AAL"
      }
      if (!("AAL" %in% names(country_df))) {
        country_aal_col <- first_numeric_col(country_df)
        if (is.na(country_aal_col)) {
          stop("Could not detect a numeric AAL column in the sample Country AAL file.")
        }
        names(country_df)[names(country_df) == country_aal_col] <- "AAL"
      }
