...
apply_plot_overrides <- function(p, input, prefix, key, defaults, is_credit = FALSE) {
  ...
  # New: X label gap (margin top) and vjust
...
secmod_server <- function(input, output, session, cart, username) {
  ...
  gallery_defaults_s5 <- reactiveVal(
    list(
      ...
      axis_text_margin_t = 5, axis_text_vjust = 0.5   # new defaults
    )
  )
  ...
  credit_penalty_plot_final <- reactive({
    ...
    p <- p + theme(
      axis.text.x = element_text(size = input$s4_axis_text %||% 12,
                                 ...
                                 hjust = 1, vjust = input$s4_axis_text_vjust %||% 0.5,
  ...
  output$sec_credit_card <- renderUI({
    ...
    div(
      ...
      div(
        ...
        # ---- Grouped controls for S4 (same as gallery) ----
        sec_control_section(
        ...
        sec_control_section(
          ...
          numericInput(session$ns("s4_axis_text_vjust"), "X label vjust", value = 0.5, min = 0, max = 1, step = 0.05, width = "80px"),
          ...
          output[[paste0("s5_dl_", key)]] <- downloadHandler(
            ...
            content = function(file) {
              ...
              defaults <- list(
                ...
                axis_text_vjust = def$axis_text_vjust %||% 0.5
              )
  ...
  output$sec_stage5_gallery_controls <- renderUI({
    ...
    sec_gallery_controls_ui(session$ns, "s5", length(rv$state_lob_plots),
                            ...
                            default_axis_text_vjust = def$axis_text_vjust %||% 0.5)
  })
  ...
  observeEvent(input$s5_apply_all, {
    ...
    def <- list(
      ...
      axis_text_vjust = input$s5_default_axis_text_vjust %||% 0.5
    )
  ...
  observeEvent(input$s6_apply_all, {
    ...
    def <- list(
      ...
      axis_text_vjust = input$s6_default_axis_text_vjust %||% 0.5
    )
    ...
    tryCatch({
      ...
      for (k in names(rv$state_lob_plots)) {
        ...
        defaults <- list(
          ...
          axis_text_vjust = def5$axis_text_vjust %||% 0.5
        )
      ...
      for (k in names(rv$modifier_plots)) {
        ...
        defaults <- list(
          ...
          axis_text_vjust = def6$axis_text_vjust %||% 0.5
        )
      ...
      report_generated_once(TRUE)   # mark that we've generated at least once
