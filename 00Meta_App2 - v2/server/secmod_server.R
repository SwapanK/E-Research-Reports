...
# -----------------------------------------------------------------------------
# PATCH: sec_status_badge — small visual refresh to match the new design language
# -----------------------------------------------------------------------------
...
secmod_server <- function(input, output, session, cart, username) {
  ...
  ## =========================================================================
  ## STAGE PROGRESS STRIP — 8 stages with lock/open icons (PATCHED)
  ## =========================================================================
  
  # -----------------------------------------------------------------------------
  # PATCH: output$sec_stepper — grouped, colour-coded progress panel
  #    Group A (blue)  = stages 1-3  "Prepare"
  #    Group B (green)  = stages 5-7  "Analyze"
  #    Group C (amber) = stages 4 & 8 "Report"
  # -----------------------------------------------------------------------------
  output$sec_stepper <- renderUI({
    ...
    tab_values <- c("stage1","stage2","stage3","stage4","stage5","stage6","stage7","stage8")
    ...
    tabs_id <- session$ns("sec_tabs")   # DOM id of the tabsetPanel, e.g. "secmod-sec_tabs"
    
    # one chip, given its index in the 8-stage sequence
    make_chip <- function(i) {
      is_done     <- statuses[i]
      is_unlocked <- unlocked[i]
      chip_class  <- "sec2-chip"
      icon_obj    <- if (is_done) {
        ...
        chip_class <- paste(chip_class, "is-open")
        icon("circle")
      }
      tags$div(
        class = chip_class,
        title = if (!is_unlocked) "Complete Stage 3 (Min / max) to unlock" else labels[i],
        onclick = if (is_unlocked) sprintf("$('#%s a[data-value=\"%s\"]').tab('show');",
                                           tabs_id, tab_values[i]) else NULL,
        span(class = "sec2-chip-icon", icon_obj),
        labels[i]
      )
    }
    ...
    div(
      class = "sec2-progress",
      div(
        class = "sec2-progress-head",
        span(class = "sec2-progress-title", icon("chart-line"), "Assessment progress"),
        span(class = "sec2-progress-meta", sprintf("%d of %d stages \u00b7 %d%%", n_done, n, pct))
      ),
      div(
        class = "sec2-track",
        div(class = "sec2-track-fill", style = sprintf("width:%s%%;", pct))
      ),
      div(
        class = "sec2-groups",
        # ---- Group A: Prepare (stages 1-3) ----
        div(
          class = "sec2-group sec2-group--a",
          div(class = "sec2-group-head",
              span(class = "sec2-group-label", icon("database"), "Prepare"),
              span(class = "sec2-group-count", sprintf("%d/3", sum(statuses[1:3])))
          ),
          div(class = "sec2-chips", lapply(1:3, make_chip))
        ),
        # ---- Group B: Analyze (stages 5-7) ----
        div(
          class = "sec2-group sec2-group--b",
          div(class = "sec2-group-head",
              span(class = "sec2-group-label", icon("shield-halved"), "Analyze"),
              span(class = "sec2-group-count", sprintf("%d/3", sum(statuses[5:7])))
          ),
          div(class = "sec2-chips", lapply(5:7, make_chip))
        ),
        # ---- Group C: Report (stages 4 & 8) ----
        div(
          class = "sec2-group sec2-group--c",
          div(class = "sec2-group-head",
              span(class = "sec2-group-label", icon("file-export"), "Report"),
              span(class = "sec2-group-count", sprintf("%d/2", sum(statuses[c(4,8)])))
          ),
          div(class = "sec2-chips", lapply(c(4,8), make_chip))
        )
      ),
      div(
        class = "sec2-footnote",
        icon("lightbulb"),
        "Stage 4 unlocks after Stage 3 \u00b7 Stage 8 unlocks after Stages 5, 6 & 7 are all complete."
      )
    )
  })
  ...
  # -----------------------------------------------------------------------------
  # PATCH: Real lock / unlock enforcement on the two report-download flows
  #   (disables the Generate buttons + drives the 3-state download button via
  #    the 'secmod-dl-state' custom message handled in secmod_ui.R)
  # -----------------------------------------------------------------------------
  
  # --- Stage 4: default report (unlocks after Stage 3 / min-max) ---
  observe({
    stage3_done <- !is.null(rv$table_minmax_USA)
    
    if (isTRUE(stage3_done)) {
      shinyjs::enable(session$ns("sec_generate_default"))
      shinyjs::removeClass(id = session$ns("sec_generate_default"), class = "state-locked")
    } else {
      shinyjs::disable(session$ns("sec_generate_default"))
      shinyjs::addClass(id = session$ns("sec_generate_default"), class = "state-locked")
    }
    
    dl_state <- if (isTRUE(rv$default_report_generated)) {
      "state-active"
    } else if (isTRUE(stage3_done)) {
      "state-ready"
    } else {
      "state-locked"
    }
    
    # ----- ADDED: Enable/disable the actual download button -----
    if (isTRUE(rv$default_report_generated)) {
      shinyjs::enable(session$ns("sec_dl_default"))
    } else {
      shinyjs::disable(session$ns("sec_dl_default"))
    }
    
    session$sendCustomMessage("secmod-dl-state", list(id = session$ns("sec_dl_default"), state = dl_state))
  })
  
  # --- Stage 8: customized report (unlocks after Stages 5, 6 & 7) ---
  observe({
    stage567_done <- !is.null(rv$credit_penalty_plot) &&
      !is.null(rv$state_lob_plots) &&
      !is.null(rv$modifier_plots)
    
    if (isTRUE(stage567_done)) {
    ...
    session$sendCustomMessage("secmod-dl-state", list(id = session$ns("sec_dl_customized"), state = dl_state))
  })
  ...
  observeEvent(input$sec_generate_default, {
    ...
    tryCatch({
      ...
      # ----- REMOVED dead runjs lines that manipulated the download button class.
      # Only keep the arrow animation (if desired) – but we'll remove those too
  ...
  observe({
    if (!isTRUE(rv$default_report_generated)) {
      # ----- REMOVED dead runjs that removed 'active' class -----
      # The state is now purely controlled by the CSS class (state-locked/ready/active)
      # and the shinyjs enable/disable, so nothing needed here.
    }
  })
  ...
  credit_penalty_plot_final <- reactive({
    ...
    p <- p + theme(
      ...
      strip.background = element_blank(),   # <-- removes grey box entirely
  ...
  observeEvent(input$sec_generate_customized, {
    ...
    tryCatch({
      ...
      # ----- REMOVED dead runjs that added 'active' class to download button -----
