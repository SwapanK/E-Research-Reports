...
# -----------------------------------------------------------------------------
# Helper: one workflow pipeline step (used in the "How it flows" demo)
# -----------------------------------------------------------------------------
home_flow_step <- function(ns, step_no, icon_name, title, desc, input_id, accent = "blue") {
  tags$button(
    id = ns(input_id),
    class = paste0("home-flow-step home-flow-step--", accent),
    onclick = sprintf("Shiny.setInputValue('%s', Math.random());", ns(input_id)),
    type = "button",
    div(class = "home-flow-step-no", step_no),
    div(class = "home-flow-step-icon", icon(icon_name)),
    div(class = "home-flow-step-title", title),
    div(class = "home-flow-step-desc", desc)
  )
}
    ...
    div(
      ...
      div(
        ...
        div(
          ...
          img(src = "GallagherRe_StackedLarge-3D.png", class = "home-hero-logo"),
      ...
      div(
        ...
        div(class = "home-section-head",
            div(class = "home-section-eyebrow", "END-TO-END PIPELINE"),
            h2(class = "home-section-title", "How your workflow flows"),
            p(class = "home-section-sub", "Four connected stages \u2014 click any stage to jump straight in.")
        ),
        div(
          class = "home-flow",
          home_flow_step(ns, "01", "file-import",  "Prepare Inputs",  "Build standardized Location & Account test files.", "go_flow_prep",    "blue"),
          div(class = "home-flow-connector"),
          home_flow_step(ns, "02", "database",      "Extract Results", "Pull ExposureSet / ResultSet data from RMS & Verisk.", "go_flow_extract", "teal"),
          div(class = "home-flow-connector"),
          home_flow_step(ns, "03", "shield-halved", "Analyze Secmod",  "Run sensitivity analysis & style the plot gallery.", "go_flow_secmod",  "amber"),
          div(class = "home-flow-connector"),
          home_flow_step(ns, "04", "file-export",   "Export Report",   "Assemble saved plots into PPTX, DOCX or HTML.", "go_flow_cart",     "green")
        )
      ),
      ...
      div(
        class = "home-footer",
        div(class = "home-footer-brand", img(src = "GallagherRe_StackedLarge-3D.png", class = "home-footer-logo")),
