...
sample_paths_input <- function(vendor, module) {
  ...
  if (vendor == "Moody's" && module == "Vulnerability Sensitivity") {
    comb <- paste0(base, "vulnerability_sensitivity_rms.csv")
  } else if (vendor == "Moody's" && module == "Secondary Modifiers") {
...
## -----------------------------------------------------------------------
## secmod_sample_paths(vendor)
## -----------------------------------------------------------------------
## Returns the demo/sample file paths for the Secondary Modifiers (SecMod)
## module, keyed off the selected vendor. Used by:
##   - server/secmod_server.R: downloadHandler outputs (sec_dl_sample_state,
##     sec_dl_sample_country, sec_dl_sample_mapping) and the
##     observeEvent(input$sec_load_demo) demo-loading logic.
## Returns a list with elements: state, usa, mapping.
##
## NOTE: the Verisk sample files must exist in data/secmod_sample/ with the
## exact filenames referenced below (mind the space before "_verisk").
## -----------------------------------------------------------------------
secmod_sample_paths <- function(vendor) {
  base <- "data/secmod_sample/"

  if (vendor == "Moody's") {
    list(
      state   = paste0(base, "AllPerils_secmod_average_state_HDv1.csv"),
      usa     = paste0(base, "AllPerils_secmod_average_USA_HDv1.csv"),
      mapping = paste0(base, "modifer,name.csv")
    )
  } else if (vendor == "Verisk") {
    list(
      state   = paste0(base, "AllPerils_secmod_average_state_HDv1 _verisk.csv"),
      usa     = paste0(base, "AllPerils_secmod_average_USA_HDv1 _verisk.csv"),
      mapping = paste0(base, "modifer,name _verisk.csv")
    )
  } else {
    ## Fallback: default to Moody's paths if vendor is unset/unrecognized
    list(
      state   = paste0(base, "AllPerils_secmod_average_state_HDv1.csv"),
      usa     = paste0(base, "AllPerils_secmod_average_USA_HDv1.csv"),
      mapping = paste0(base, "modifer,name.csv")
...
build_moodys_vulnerability <- function(latlon, combination, args) {
  ...
  meta <- vendor_meta("Moody's", args$peril)
  short <- module_short(args$module)
  accountsuffix <- paste0(short, "_", args$subperil, meta$abb, args$suffix)
  fn <- build_output_names("Moody's", args$version, args$module, args$country, args$peril, args$subperil)
...
build_files <- function(vendor, module, version, country, peril, subperil, suffix,
  ...
  if (vendor == "Moody's" && module == "Secondary Modifiers") {
