...
# =============================================================================
# ---- PER-PLOT X-AXIS CLASS REORDER / RENAME (v2 - modal-based) ------------
# =============================================================================
# v1 rendered the Order/Name/Rename table inline (DT::renderDT) inside the
# card's collapsible override panel. That panel is a CSS grid that starts
# display:none, and DataTables' width calculation doesn't handle that
# reliably - columns collapse to 0px and never repaint until something
# forces a browser reflow.
#
# v2: vul_class_reorder_ui() below is now just a section with one button
# that opens a modalDialog (vul_class_order_modal_ui()). Inside the modal,
# the table is a plain HTML <table> - no DT - and all reordering/renaming
# happens client-side, in memory, in the JS embedded in
# vul_class_order_modal_ui() itself. Nothing talks back to Shiny until the
# modal's Apply button fires a single Shiny.setInputValue() with the final
# order+labels as JSON; Vulsen_server.R's setup_class_order_manager()
# observes that input, commits it to class_order_committed[[ck]], and
# closes the modal. There is no more server-side staging reactive, no
# renderDT/renderUI pair, and no per-click/per-keystroke round trip.
#
# Id convention (must match Vulsen_server.R's setup_class_order_manager()):
#   {prefix}_class_order_open_{safe_key}    actionButton, opens the modal
#   {prefix}_class_order_apply_{safe_key}   hidden input, set once via the
#                                            modal's Apply button JS -
#                                            value is a JSON string:
#                                            [{"order":1,"name":...,"rename":...,"visible":true}, ...]
# =============================================================================

#' UI shell for the per-plot class reorder/rename block: a titled section
#' containing a single "open the editor" button. Called at the end of
#' vul_plot_card_gallery(), inside that card's collapsible override panel,
#' so it sits after Data Label Properties and before the panel closes.
#'
#' @param ns Module namespace function.
#' @param key The plot's Classification label (e.g. "CC") - used for the
#'   hint text and the button label.
#' @param prefix,safe_key Same prefix/sanitized-key pair already threaded
#'   through the rest of vul_plot_card_gallery() / vul_apply_overrides().
vul_class_reorder_ui <- function(ns, key, prefix, safe_key) {
  open_id <- paste0(prefix, "_class_order_open_", safe_key)

  sec_control_section(
    "X-Axis Class Order & Labels",
    section_class = "sec2-control-section vul-class-order-section",
    div(
      style = "grid-column: 1 / -1;",
      class = "vul-class-order-card",

      sec_hint(paste0(
        "Reorder, rename, or hide the \u201c", key, "\u201d classes on this plot's ",
        "x-axis in a popup, preview the result, then click Apply to update the plot."
      )),

      div(
        class = "vul-legend-toolbar-row vul-class-order-apply-row",
        actionButton(
          ns(open_id),
          paste0("Edit Order & Labels (", key, ")"),
          icon = icon("arrows-up-down"),
          class = "sec2-btn sec2-btn-primary"
        )
      )
    )
  )
...
#' Build the "X-Axis Class Order & Labels" modalDialog for one plot card.
#' Called from Vulsen_server.R's setup_class_order_manager() every time the
#' open button is clicked, seeded with the card's current committed
#' order/rename/visible state.
#'
#' Everything inside the modal - the Order/Name/Rename/Show table, the
#' arrow reordering, the Rename text inputs, the Show/Hide eye toggle, and
#' the "A | L | R | ..." preview - is plain HTML built and re-rendered
#' entirely by the JS below, against a plain in-memory array. Nothing
#' touches Shiny until Apply is clicked. The modal's DOM is created fresh
#' on every open and torn down by removeModal()/Cancel, so the click/input
#' listeners are bound directly to this modal's own elements - no
#' document-level delegation, no "already-bound" guard needed (unlike the
#' old v1 arrow-click script).
#'
#' @param ns Module namespace function (pass session$ns from the server).
#' @param key The plot's Classification label (e.g. "CC") - modal title/
#'   hint text only.
#' @param apply_input_id The *unnamespaced* Shiny input id the Apply
#'   button's JS should Shiny.setInputValue() into - must be the same id
#'   Vulsen_server.R's observeEvent(input[[apply_id]], ...) listens on.
#' @param order_df Current committed order/name/rename/visible data.frame
#'   for this card (class_order_committed[[ck]]) - what the modal is
#'   seeded with. A missing `visible` column (older committed states) is
#'   treated as all-visible.
vul_class_order_modal_ui <- function(ns, key, apply_input_id, order_df) {
  has_visible <- "visible" %in% names(order_df)
  df <- order_df[order(order_df$order), c("name", "rename"), drop = FALSE]
  df$visible <- if (has_visible) {
    v <- order_df$visible[order(order_df$order)]
    v[is.na(v)] <- TRUE
    v
  } else {
    TRUE
  ...
  seed_json <- jsonlite::toJSON(
    lapply(seq_len(nrow(df)), function(i) {
      list(name = df$name[i], rename = df$rename[i], visible = isTRUE(df$visible[i]))
    }),
    auto_unbox = TRUE
  )

  seed_id      <- ns("vul_class_order_modal_seed")
  tbody_id     <- ns("vul_class_order_modal_tbody")
  preview_id   <- ns("vul_class_order_modal_preview")
  warn_id      <- ns("vul_class_order_modal_warn")
  apply_btn_id <- ns("vul_class_order_modal_apply_btn")
  apply_target <- ns(apply_input_id)

  shiny::modalDialog(
    title = paste0("X-Axis Class Order & Labels \u2014 ", key),
    size = "m",
    easyClose = TRUE,
    fade = TRUE,

    sec_hint(paste0(
      "Reorder, rename, or hide the \u201c", key, "\u201d classes on this plot's x-axis. ",
      "Use the arrows to reorder, edit Rename to relabel, click the eye to show/hide a class, ",
      "watch the preview update, then click Apply."
    )),

    tags$script(type = "application/json", id = seed_id, HTML(as.character(seed_json))),

    div(
      class = "vul-legend-table-wrap vul-class-order-table-wrap",
      tags$table(
        class = "display vul-class-order-modal-table",
        tags$thead(
          tags$tr(
            tags$th("Order"),
            tags$th(HTML("<i class='fa fa-lock'></i> Name")),
            tags$th(HTML("<i class='fa fa-pencil'></i> Rename")),
            tags$th(class = "vul-order-show-th", HTML("<i class='fa fa-eye'></i> Show"))
          )
        ),
        tags$tbody(id = tbody_id)
      )
    ),

    div(id = warn_id, class = "vul-order-warn", style = "display:none;",
        "At least one class must stay visible."),

    div(
      class = "vul-legend-toolbar-group",
      div(class = "vul-legend-toolbar-label", "Preview"),
      div(id = preview_id, class = "vul-order-preview-row")
    ),
    ...
    tags$script(HTML(sprintf(
      "(function() {
         var seedEl   = document.getElementById('%s');
         var tbody    = document.getElementById('%s');
         var preview  = document.getElementById('%s');
         var warnEl   = document.getElementById('%s');
         var applyBtn = document.getElementById('%s');
         var applyInputId = '%s';
         var state = JSON.parse(seedEl.textContent);
         state.forEach(function(row) { row.visible = (row.visible !== false); });
         ...
         function visibleCount() {
           return state.reduce(function(n, row) { return n + (row.visible ? 1 : 0); }, 0);
         }

         function renderPreview() {
           var chips = state.map(function(row) {
             var label = (row.rename && String(row.rename).trim().length) ? row.rename : row.name;
             var cls = 'vul-order-preview-chip' + (row.visible ? '' : ' vul-order-preview-chip-hidden');
             return '<span class=\"' + cls + '\">' + escapeHtml(label) + '</span>';
           });
           preview.innerHTML = chips.length
             ? chips.join('<span class=\"vul-order-preview-sep\">|</span>')
             : '<span class=\"vul-order-preview-empty\">No classes to preview.</span>';
         }

         function renderRows() {
           var html = '';
           state.forEach(function(row, i) {
             var rowCls = row.visible ? '' : ' class=\"vul-order-row-hidden\"';
             var eyeCls = row.visible ? 'fa-eye' : 'fa-eye-slash';
             var eyeTitle = row.visible ? 'Hide this class' : 'Show this class';
             html += '<tr' + rowCls + '>' +
               '<td class=\"dt-right\"><div class=\"vul-order-arrows\">' +
                 '<button type=\"button\" class=\"vul-order-arrow vul-order-arrow-up\" data-idx=\"' + i + '\" data-dir=\"up\" title=\"Move up\">&#9650;</button>' +
                 '<span class=\"vul-order-index\">' + (i + 1) + '</span>' +
                 '<button type=\"button\" class=\"vul-order-arrow vul-order-arrow-down\" data-idx=\"' + i + '\" data-dir=\"down\" title=\"Move down\">&#9660;</button>' +
               '</div></td>' +
               '<td style=\"background-color:#f5f5f5;color:#888;cursor:default;\">' + escapeHtml(row.name) + '</td>' +
               '<td style=\"cursor:text;\"><input type=\"text\" class=\"form-control vul-order-rename-input\" data-idx=\"' + i + '\" value=\"' + escapeAttr(row.rename) + '\" ' + (row.visible ? '' : 'disabled') + '></td>' +
               '<td class=\"vul-order-show-td\"><button type=\"button\" class=\"vul-order-eye-btn\" data-idx=\"' + i + '\" title=\"' + eyeTitle + '\" aria-pressed=\"' + row.visible + '\"><i class=\"fa ' + eyeCls + '\"></i></button></td>' +
             '</tr>';
           });
         ...
         tbody.addEventListener('click', function(e) {
           var arrowBtn = e.target.closest('.vul-order-arrow');
           if (arrowBtn) {
             var idx = parseInt(arrowBtn.getAttribute('data-idx'), 10);
             var dir = arrowBtn.getAttribute('data-dir');
             var target = dir === 'up' ? idx - 1 : idx + 1;
             if (target < 0 || target >= state.length) return;
             var tmp = state[idx]; state[idx] = state[target]; state[target] = tmp;
             renderRows();
             return;
           }
           var eyeBtn = e.target.closest('.vul-order-eye-btn');
           if (eyeBtn) {
             var eIdx = parseInt(eyeBtn.getAttribute('data-idx'), 10);
             var row = state[eIdx];
             if (row.visible && visibleCount() <= 1) {
               warnEl.style.display = 'block';
               window.setTimeout(function() { warnEl.style.display = 'none'; }, 2500);
               return;
             }
         ...
         tbody.addEventListener('input', function(e) {
           var inp = e.target;
           if (!inp.classList.contains('vul-order-rename-input')) return;
           var idx = parseInt(inp.getAttribute('data-idx'), 10);
           state[idx].rename = inp.value;
           renderPreview();
         });
         ...
         applyBtn.addEventListener('click', function() {
           ...
           var payload = state.map(function(row, i) {
             var rn = (row.rename && String(row.rename).trim().length) ? row.rename : row.name;
             return { order: i + 1, name: row.name, rename: rn, visible: !!row.visible };
           });
           Shiny.setInputValue(applyInputId, JSON.stringify(payload), {priority: 'event'});
         });

         renderRows();
       })();",
      seed_id, tbody_id, preview_id, warn_id, apply_btn_id, apply_target
    )))
  )
}
...
vul_gallery_controls_ui <- function(
    ...
    default_panel_gap_px = 8,
    default_axis_text = 9,
    default_x_rotation = 0,
    default_x_vjust = 0.5,
    default_plot_title = 16,
    default_strip_text = 9,
...
vul_plot_card_gallery <- function(
  ...
  div(
    ...
    div(
      ...
      # X-Axis Class Order & Labels - reorder/rename table + preview + Apply.
      # Placed last so it reads as "everything above is live/instant, this
      # is the one section that needs an explicit Apply" (same convention
      # as the Legend Configuration Manager's Download JSON row sitting
      # after its preview).
      vul_class_reorder_ui(ns, key, prefix, safe_key)
    ),
    # ---- Force swatch sync after this UI is rendered ----
    tags$script(HTML("setTimeout(secmodSyncAllSwatches, 100);"))
  )
