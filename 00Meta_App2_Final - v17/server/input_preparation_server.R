...
inputPrepServer <- function(input, output, session) {
  ...
  observeEvent(input$use_samples, {
    use_samples(TRUE)
    data_loaded_confirmed(TRUE)
    showNotification("Using bundled sample files for the current vendor/module selection.", type = "message", duration = 5)
  })
  ...
  observeEvent(input$create_files, {
    tryCatch({
      ...
      generated(res)
      showNotification("Input files generated successfully.", type = "message", duration = 5)
    }, error = function(e) {
      generated(NULL)
