...
plot_to_data_uri <- function(plot_obj, width = 8, height = 4.5, dpi = 120) {
  ...
  uri <- tryCatch({
    ...
    # Surface the real failure instead of letting it kill the whole
    # cart_items_ui render (or fail silently). Check the R console/log
    # right after clicking "Add to cart" to see this message.
    message("plot_to_data_uri() FAILED (w=", width, " h=", height, " dpi=", dpi, "): ",
            conditionMessage(e))
    NULL
  })
      ...
      lapply(seq_along(items), function(i) {
        ...
        div(
          ...
          if (!is.null(item$plot)) {
            uri <- plot_to_data_uri(item$plot, width = w, height = h, dpi = dpi)
            if (!is.null(uri)) {
              tags$img(
                src   = uri,
              ...
              div(
                class = "cart-image-error",
                style = "padding:16px; text-align:center; color:#EF4444; font-size:13px;",
                icon("triangle-exclamation"), " Could not render this plot image. Check the app log for details."
              )
            }
          },
