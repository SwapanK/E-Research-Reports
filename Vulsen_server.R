...
Vulsen_server <- function(id, cart, username) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    shiny::observeEvent(input$vul_cart_click, {
      ...
      # The button sends the sanitized id (safe_key); rv$plots is keyed by
      # the raw category name, so resolve back to it the same way the ids
      ...
      local({
        ...
        shiny::observeEvent(input[[paste0(grp, "_apply_all")]], {
          ...
          for (key in plot_names) {
            ...
            shiny::updateSelectInput(session, paste0(grp, "_x_rotation_", safe_key), selected = def$x_rotation)
