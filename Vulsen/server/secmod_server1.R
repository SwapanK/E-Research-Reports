...
sec_plot_card_gallery <- function(key, label, prefix) {
  ...
  div(
    ...
    plotOutput(paste0(prefix, "_plot_", key), height = "320px"),
...
secmod_server <- function(input, output, session, cart, username) {
  ...
  output$sec_stepper <- renderUI({
    ...
    div(
      style = "display:flex; align-items:center; gap:4px; overflow-x:auto; padding:4px 0;",
      lapply(seq_along(labels), function(i) {
        done <- statuses[i]
        tagList(
          div(
            style = "display:flex; align-items:center; gap:6px; flex:1; min-width:120px;",
            span(
              style = sprintf(
                "width:22px;height:22px;border-radius:50%%;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:600;background:%s;color:%s;flex-shrink:0;",
                if (done) "#E1F5EE" else "#F1EFE8", if (done) "#085041" else "#888780"
              ),
              if (done) icon("check") else as.character(i)
            ),
            span(style = "font-size:12px;", labels[i])
          ),
          if (i < length(labels)) div(style = "width:16px;height:1px;background:#e2e8f0;flex-shrink:0;")
        )
      })
    )
  })
  ...
  output$sec_stage5_gallery <- renderUI({
    ...
    tagList(lapply(keys, function(k) sec_plot_card_gallery(k, gsub("_", " ", k), "s5")))
  })
  ...
  output$sec_stage6_gallery <- renderUI({
    ...
    tagList(lapply(keys, function(k) sec_plot_card_gallery(k, rv$modifier_labels[[k]], "s6")))
  })
