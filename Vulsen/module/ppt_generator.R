  ...
  for (i in seq_along(cart_items)) {
    ...
    if (!is.null(item$plot)) {
      ...
      ggplot2::ggsave(
        filename = img_path,
        plot     = item$plot,
        width    = plot_w, height = plot_h, dpi = 160, bg = "white"
      )

      doc <- officer::ph_with(
        doc,
        value    = officer::external_img(img_path, width = plot_w, height = plot_h),
        location = officer::ph_location(left = plot_margin, top = 1.3, width = plot_w, height = plot_h)
      )
