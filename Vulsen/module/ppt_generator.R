...
.add_header <- function(doc, tmp_dir, title_text, badge_text = NULL,
                         slide_w = 10, band_h = 0.95, th = .vulsen_theme) {
...
## -----------------------------------------------------------------------
.add_footer <- function(doc, tmp_dir, page_no, slide_w = 10, slide_h = 7.5,
...
generate_cart_ppt <- function(
  ...
  th <- theme
  tmp_dir <- tempdir()
  slide_w <- 10; slide_h <- 7.5   # default officer template dims (4:3)
  ...
  company_run <- officer::ftext(toupper(company_name), officer::fp_text(
    font.size = 15, bold = TRUE, color = th$accent, font.family = th$font
  ))
  doc <- officer::ph_with(
    doc,
    value    = officer::block_list(officer::fpar(company_run,
                 fp_p = officer::fp_par(text.align = "center", padding = 0))),
    location = officer::ph_location(left = 0.5, top = 2.15, width = slide_w - 1.0, height = 0.45)
  )
  ...
  for (i in seq_along(cart_items)) {
    ...
    if (!is.null(item$plot)) {
      ...
      ggplot2::ggsave(
        filename = img_path,
        plot     = item$plot,
        width    = 8.7, height = 5.75, dpi = 160, bg = "white"
      )

      doc <- officer::ph_with(
        doc,
        value    = officer::external_img(img_path, width = 8.7, height = 5.75),
        location = officer::ph_location(left = 0.65, top = 1.3, width = 8.7, height = 5.75)
      )
